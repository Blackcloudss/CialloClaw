...
- [ ] `Task / Run / Step / Event / ToolCall / DeliveryResult / Artifact / ApprovalRequest / AuditRecord / RecoveryPoint` 这些共享对象在 `packages/protocol`、Go runtime、前端消费层三边字段一致。
...
- [ ] SQLite + WAL 结构化运行态可用。
- [ ] `tasks / runs / task_steps / events / tool_calls / delivery_results / approval_requests / audit_records / recovery_points` 至少完成最小可用读写。（owner-5 P0-2 已补齐 `tasks / task_steps` 一等存储与 `task.list / task.detail` 的正式表优先回退读取，剩余读侧仍需继续去快照化）
...
- [ ] 至少一条长结果自动分流到文档或文件：`delivery_result / artifact / open_action` 在前后端完整承接。
...
- [ ] 文件交付、打开文件、定位目录、workspace document 打开动作形成稳定体验（后端 `agent.task.artifact.open` / `agent.delivery.open` 已统一，待前端接入最终打开动作）。
...
- [ ] 继续把 4 号剩余工作按“执行闭环、产品承接、运行时稳定化、联调验收、测试回归”五类维护，避免再把“已有基础能力”和“真实剩余项”混写在一起。
...
- [ ] 稳定 OpenAI Responses API 接入，并把 planner/tool-calling 路径接到正式主链路配置。
- [ ] 稳定文件读写、命令执行、网页读取等基础工具，并把输出统一回流到 `tool_call / event / delivery_result` 链。
- [ ] 补齐 SQLite + WAL、任务运行态、记忆层、治理层最小读写，确保 `runs / steps / events / tool_calls / trace_records / eval_snapshots` 结构化可用。
- [ ] 落地风险分级、授权、审计、恢复点最小闭环，并确保至少一条高风险动作真跑通。
- [ ] 完成 Workspace / Artifact 最小落盘，并稳定支持 `open_file / reveal_in_folder / workspace_document` 等打开动作所需后端能力。
- [ ] 让屏幕 / 页面感知、OCR / Playwright、插件运行态等能力按优先级推进，并把页面/窗口/可见文本信号接入主链路上下文。
