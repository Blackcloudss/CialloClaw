...
- [ ] `input.page_context` 以及 `context.page`、`context.screen`、`context.behavior` 中与页面标题、窗口标题、可见文本、屏幕摘要、停留和切换有关的信号，需要继续作为 4 号主责维护的正式输入上下文，不允许各端再发明平行命名。
- [x] Orchestrator / Intent 已补齐“自然语言触发视觉型任务”的主链路承接，例如“帮我看看这个页面的报错”“看看当前屏幕上哪里出错了”当前都可落入正式 task，而不是要求前端先手工拼显式 intent。
- [ ] Delivery / 查询装配仍需明确区分模型结论、截图证据、OCR 文本、引用片段和审计信息，保证任务详情消费的是正式 `artifact / citation / delivery_result / approval_request / authorization_record / audit_record`，而不是裸 worker 输出。
