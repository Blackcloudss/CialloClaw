...
- [ ] `Task.status`、`approval_request.status`、`bubble_message`、`delivery_result`、前端 route state 这些正式对象与局部状态分层已经在代码中收清，不再新增平行模型。
...
- [ ] 文本选中入口至少完整跑通一次：`TextPattern / 真实选区承接 -> agent.task.start -> agent.task.confirm -> task detail / bubble result`。
...
- [ ] 把悬浮球单击 / 双击 / 长按 / 上滑 / 下滑在 `ShellBallCoordinator`、`VoicePreview`、`Bubble` 状态机里稳定下来。
...
- [ ] 把 `agent.task.events.list`、`agent.task.steer`、`loop.*` / `task.steered` 正式通知继续联调到前端任务详情或调试视图。
- [ ] 把至少一条真实执行链跑到前端承接闭环，覆盖任务详情结果区或 artifact 打开动作。
