...
- [x] 已为 task detail runtime event 视图接入 `agent.task.events.list` 的 `type / run_id / created_at_from / created_at_to` 正式筛选合同，当前前后端都支持按事件类型、执行 run 和时间范围过滤运行时事件。
...
- [x] 为 P3 的多模型配置、社区 Skills、感知包扩展预留正确边界，而不是现在提前扩散。（owner-5 已补 provider registry 预留、多模型 descriptor 边界，以及 `skill_manifests / blueprint_definitions / prompt_template_versions` 最小持久层预留，但仍未提前扩散多 provider、社区 skill 安装或 blueprint 产品能力。）
