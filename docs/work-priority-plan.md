...
- [ ] 继续把 Agent Loop 的真实执行链补齐到 `tool_call / event / delivery_result` 全链回流，不再只停留在 task detail runtime 观察层。
- [ ] 选择至少一条 owner-4 可控的直接执行链作为稳定样板，优先覆盖 `agent.task.start` / `agent.input.submit` 直达执行、正式 delivery 生成、artifact 落盘与前端打开承接。
- [ ] 将 4号 剩余工作继续拆成“运行时落盘闭环”“交付结果查询闭环”“前端真实承接闭环”三个可独立验证的小任务，避免后续以笼统的 Agent Loop 完善名义重复改同一层。
...
- [ ] 第一优先级剩余闭环：至少一条真实执行链打穿前端正式承接；至少一种 artifact 类型完成真实落盘与打开/定位闭环；至少一条长结果自动分流形成正式交付闭环；至少一条高风险动作链路完成授权 / 恢复 / 审计验收。
- [ ] 第二优先级产品收口：task detail 增强为主链路收口页；dashboard 升级为运行态工作台；`loop.*` / `task.steered` 正式消费链冻结；task.control 与 task.steer 的边界梳理完成。
- [x] dashboard 首页焦点任务已开始消费运行态摘要，当前至少能展示最近运行事件、待消费 steering 提示与等待授权数量；完整运行态工作台仍需继续补齐更细粒度承接。
- [ ] 第三优先级运行时稳定化：retry policy 细化到错误类型级别；steering policy 冻结；runtime history / compaction / stop reason 语义稳定；repeated tool loop / doom loop 检测与说明补强。
- [ ] 第四优先级联调覆盖：文本选中、文件拖拽、语音入口、悬浮球输入入口分别完成真实联调验收；巡检 / Notepad 升级任务链路完成 task detail / delivery / dashboard 的一致性收尾。
- [ ] 第五优先级测试与观察：补齐跨端最终验收 case；继续补 runtime 稳定性回归矩阵；完善 Agent Loop 调试 / 观察能力；收尾 shell-ball 测试与联调环境问题。
