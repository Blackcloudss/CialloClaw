...
* 悬浮球允许用户自由拖拽；拖拽过程中可短暂贴近或越过屏幕边缘，但在用户松开鼠标后必须立即回弹到当前屏幕的可用工作区内，不得停留在屏幕边界之外。
...
* 文件拖拽用于把当前文件加入下方轻量输入区的附件队列，并与“附加文件”按钮上传的附件保持同等级。系统接收到文件后，不直接发送请求；用户可继续补充一句说明，再由用户手动发送，之后才进入意图确认与执行流程。
...
* 当用户进入仪表盘阶段后，引导窗口应恢复透明宿主，只保留说明卡本身，不再持续叠加整屏灰色遮罩。
...
* 对于文件场景，系统在用户手动发送前只负责承接附件与补充说明；用户明确发送后，系统再结合文件内容与当前上下文进入意图判断。
...
当用户拖拽悬浮球、悬浮球自动贴边，或屏幕约束导致气泡需要换边时，气泡应以短时、平滑的位移动画完成换位，而不是突然跳变到新的停靠位置。
