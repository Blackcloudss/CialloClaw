...
| --- | --- | --- |
| 第 4 章“总体架构” | `2.4.4`、`3.0`、`4.0` | 先看总览桥接，再看前后端总览图 |
| 第 5 章“关键架构对象与边界” | `2.4.5`、`3.7`、`4.2`、`5.11` | 先看对象边界，再看功能域和交付分流 |
| 第 6 章“逻辑分层与协作边界” | `2.4.6`、`3.1~3.7`、`4.1~4.5` | 前端层和后端层的职责、输入输出、边界在这里逐层展开 |
| 第 7 章“核心链路设计” | `5.0~5.11`、`6.1~6.11` | 第 5 章看系统级主链，第 6 章看功能级时序 |

阅读建议：

- 想先理解“系统怎么分层”，从 `2.4.4~2.4.7` 进入。
...
| 模块 | 详细章节 | 核心职责摘要 |
| --- | --- | --- |
| 接口接入层 | `4.1` | JSON-RPC 收口、对象锚定、错误包装、事件推送 |
| Harness 内核层 | `4.2` | 任务编排、上下文归一、意图建议、状态推进、交付与插件系统 |
| 能力接入层 | `4.3` | 模型、工具、worker、sidecar、RAG 与屏幕/视频能力接入 |
| 治理与反馈层 | `4.4` | 风险评估、审查、Trace / Eval、恢复、预算和边界治理 |
| 平台与执行适配层 | `4.5` | 文件系统、系统能力、执行后端和跨平台抽象 |
