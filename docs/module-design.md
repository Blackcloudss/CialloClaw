...
- **续接分类**：`maybeContinueExistingTask()` 先走确定性与启发式规则，不足时才调用模型做 coarse-grained continuation classification；当 `confirm_required = true` 时，只允许带有任务特定证据（如 lineage、共享文件、同一页面 / 窗口 / 对象锚点）的文件、选区、错误等结构化补充证据续接到 `waiting_input / confirming_intent` 这类待用户补充的任务，不能只因为输入是结构化对象就续接，不能续接正在执行的任务，也不能自动执行；悬浮球默认入口锚点（如 `desktop / local://shell-ball`）只表示补充证据从哪里进入系统，不应覆盖或冲突于原 task 的真实页面 / 应用锚点。
