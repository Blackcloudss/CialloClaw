...
- **续接分类**：`maybeContinueExistingTask()` 先走确定性与启发式规则，不足时才调用模型做 coarse-grained continuation classification；当 `confirm_required = true` 时，只允许文件、选区、错误等结构化补充证据续接到 `waiting_input / confirming_intent` 这类待用户补充的任务，不能续接正在执行的任务，也不能自动执行；悬浮球默认入口锚点（如 `desktop / local://shell-ball`）只表示补充证据从哪里进入系统，不应覆盖或冲突于原 task 的真实页面 / 应用锚点。
...
**实现说明**：此链路是所有近场承接动作的统一模板，文本选中、拖拽文件、错误信息承接和主动机会承接都应落在同一确认 / 执行骨架上；带明确说明的文件任务可以直接进入治理与执行，裸文件仍进入确认或补充输入，结构化补充证据可续接到正在等待用户输入的原 task。
...
**实现说明**：无论对象来源是选中文本、拖拽文件还是主动推荐，后续都应收敛到同一套确认与执行骨架；结构化补充证据可以续接待用户补充的原 task，但不能绕过确认门禁去自动执行。
...
**实现补充**：所有对象型入口都应复用同一确认 / 执行骨架，不允许各入口维护不同的确认状态定义；带明确说明的文件任务可直接进入主执行链路，裸文件或低置信度对象进入确认 / 补充输入，结构化补充证据可挂回 `waiting_input / confirming_intent` 任务但仍不得自动执行。
